use frontend::ir::{Declaration, DeclarationData, Expr, ExprData, MatchBranch, Program, SetTarget, SetTargetData};
...
fn expr_size(e: &Expr) -> u32 {
    let size: u32 = match e.it.as_ref() {
        ...
        ExprData::If { condition, then_branch, else_branch } => expr_size(condition) + expr_size(then_branch) + expr_size(else_branch) + 1,
        ExprData::Block { declarations, expr } => declarations.iter().map(decl_size).sum::<u32>() + expr_size(expr),
        ExprData::Struct { fields, .. } => fields.iter().map(|(_, expr)| expr_size(expr)).sum::<u32>() + 1,
        ExprData::StructIdx { expr, .. } => expr_size(expr) + 1,
        ExprData::Return { expr } => expr_size(expr) + 1,
        ExprData::Match { scrutinee, branches } => expr_size(scrutinee) + branches.iter().map(branch_size).sum::<u32>(),
