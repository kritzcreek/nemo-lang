...
fn fits_test() {
    ...
    assert!(!fits(-1, 0, VecDeque::from([])));

    // If no more documents it always fits
    assert!(fits(0, 0, VecDeque::from([])));

    // ForceBreak never fits
    let doc = ForceBroken(Box::new(nil()));
    assert!(!fits(100, 0, VecDeque::from([(0, Unbroken, &doc)])));
    let doc = ForceBroken(Box::new(nil()));
    assert!(!fits(100, 0, VecDeque::from([(0, Broken, &doc)])));
    ...
    // Line always fits
    assert!(fits(0, 0, VecDeque::from([(0, Broken, &Line(100))])));
    assert!(fits(0, 0, VecDeque::from([(0, Unbroken, &Line(100))])));

    // String fits if smaller than limit
    let doc = String("Hello".into());
    assert!(fits(5, 0, VecDeque::from([(0, Broken, &doc)])));
    let doc = String("Hello".into());
    assert!(fits(5, 0, VecDeque::from([(0, Unbroken, &doc)])));
    let doc = String("Hello".into());
    assert!(!fits(4, 0, VecDeque::from([(0, Broken, &doc)])));
    let doc = String("Hello".into());
    assert!(!fits(4, 0, VecDeque::from([(0, Unbroken, &doc)])));

    // Cons fits if combined smaller than limit
    let doc = String("1".into()).append(String("2".into()));
    assert!(fits(2, 0, VecDeque::from([(0, Broken, &doc)])));
    let doc = String("1".into()).append(String("2".into()));
    assert!(fits(2, 0, VecDeque::from([(0, Unbroken, &doc,)])));
    let doc = String("1".into()).append(String("2".into()));
    assert!(!fits(1, 0, VecDeque::from([(0, Broken, &doc)])));
    let doc = String("1".into()).append(String("2".into()));
    assert!(!fits(1, 0, VecDeque::from([(0, Unbroken, &doc)])));
