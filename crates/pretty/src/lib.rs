//! Original authors are the Gleam Lang contributors (https://gleam.run)
//! This file is a modified version of the original file from the Gleam repository.
//!
...
use std::{collections::VecDeque, fmt::{Write, Result}};
use ecow::EcoString;
    ...
    while let Some((indent, mode, document)) = docs.pop_front() {
        match document {
            ...
            Document::Group(doc) => {
                ...
                // a single line.
                let group_docs = VecDeque::from([(indent, Mode::Unbroken, doc.as_ref())]);
...
impl<'a> Document<'a> {
    ...
    pub fn pretty_print(&self, limit: isize, writer: &mut impl Write) -> Result {
        let docs = VecDeque::from([(0, Mode::Unbroken, self)]);
