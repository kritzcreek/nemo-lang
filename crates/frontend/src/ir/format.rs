use lexpr::Value;
use crate::ir::{Callee, Declaration, DeclarationData, Expr, ExprData, LitData, Name, NameSupply, Program, SetTarget, SetTargetData, Ty};
...
impl Formatter<'_> {
    ...
    fn set_target(&self, set_target: &SetTarget) -> Value {
        match &set_target.it {
            ...
            SetTargetData::SetVar { name } => Value::list(vec![
                Value::symbol("set_var"),
                self.name(name),
            ])
        }
    }
    ...
    fn decl(&self, decl: &Declaration) -> Value {
        match &decl.it {
            DeclarationData::Let { binder, expr } => {
                Value::list(vec![Value::symbol("let"), self.name(binder), self.expr(expr)])
            }
            DeclarationData::Set { set_target, expr } =>
                Value::list(vec![Value::symbol("set"), self.set_target(set_target), self.expr(expr)]),
            DeclarationData::Expr { expr } => self.expr(expr),
            DeclarationData::While { condition, body } =>
                Value::list(vec![Value::symbol("while"), self.expr(condition), self.expr(body)]),
        }
    }
    ...
    fn expr(&self, e: &Expr) -> Value {
        match e.it.as_ref() {
                ...
                match func {
                    Callee::Func { name, type_args } => {
                        ...
                        if !type_args.is_empty() {
                            elems.push(Value::vector(type_args.tys().into_iter().map(|t| self.ty(t))))
                        }
                    }
            ...
            ExprData::If { condition, then_branch, else_branch } => Value::list(vec![
                Value::symbol("if"),
                self.expr(condition),
            ...
            ExprData::Struct { name, fields } => {
                ...
                elems.extend(fields.into_iter().map(|(name, expr)| Value::from((self.name(name), self.expr(expr)))));
                Value::list(elems)
            }
            ...
            ExprData::Lambda { captures, params, return_ty, body } => {
                let mut elems = vec![Value::symbol("lambda")];
                elems.push(Value::vector(captures.into_iter().map(|(n, ty)| Value::from((self.name(n), self.ty(ty))))));
                elems.push(Value::list(params.into_iter().map(|(n, ty)| Value::from((self.name(n), self.ty(ty))))));
